  ...
  return (
    <main className="flex flex-1 relative h-[87vh]">
      ...
      <div className="flex-1 bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col overflow-hidden relative">
        ...
        {/* Toolbar Overlay */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 bg-white shadow-xl border border-slate-200 rounded-full px-4 py-2 flex items-center gap-4">
          
          {/* Tools */}
          <div className="flex gap-2">
            <button
              onClick={() => { setEraserMode(false); canvasRef.current?.eraseMode(false); }}
              className={`w-9 h-9 flex items-center justify-center rounded-full transition ${!eraserMode ? 'bg-blue-100 text-blue-600' : 'text-slate-500 hover:bg-slate-100'}`}
              title="Pen"
            >
          ...
          <button onClick={handleExport} className="w-9 h-9 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 transition" title="Export Image">
        ...
        {/* Canvas Area */}
        <div className="flex-1 w-full h-full bg-slate-50 relative cursor-crosshair">
          <ReactSketchCanvas
            ref={canvasRef}
